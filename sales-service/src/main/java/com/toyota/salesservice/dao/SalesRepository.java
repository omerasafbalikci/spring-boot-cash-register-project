...
public interface SalesRepository extends JpaRepository<Sales, Long> {
    @Query("SELECT s FROM Sales s WHERE " +
            ...
            "(?3 IS NULL OR s.salesDate >= TO_TIMESTAMP(?3, 'YYYY-MM-DD HH24:MI:SS')) AND " +
            "(?4 IS NULL OR UPPER(s.createdBy) LIKE CONCAT('%', UPPER(?4), '%')) AND " +
            "(?5 IS NULL OR UPPER(s.paymentType) LIKE CONCAT('%', UPPER(?5), '%')) AND " +
            "(?6 IS NULL OR s.totalPrice >= ?6) AND " +
            "(?7 IS NULL OR s.money >= ?7) AND " +
            "(?8 IS NULL OR s.change >= ?8)")
    Page<Sales> getSalesFiltered(Long id, String salesNumber, String salesDate, String createdBy, String paymentType,
